   ...
   public class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select worlinfo.*,activity.activityName from workinfo join activity on workinfo.activityId=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过' and recommand='是' order by recommendTime desc limit 6 ";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public IEnumerable<Model.WorkInfoNo> GetMyPage(Model.WorkMyPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select* from a where num between (@oageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.WorkInfoNo>(sql, page);
            }
        }
